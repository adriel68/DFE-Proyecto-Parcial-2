...
function displayTasksTable(tasks) {
  ...
  tasks.forEach(task => {
    ...
    row.innerHTML = `
      ...
      <td>
        <button class="btn-delete" data-task-id="${task.id}">Borrar tarea</button>
      </td>
      <td>
        <button class="btn-update" data-task-id="${task.id}">Actualizar tarea</button>
      </td>
    `;
...
function getTasksData() {
  fetchAPI(`${apiURL}/users/219220991/tasks`, 'GET')
    .then(data => {
...
function getTaskData(taskId) {
  fetchAPI(`${apiURL}/users/219220991/tasks/${taskId}`, 'GET')
    .then(data => {
...
function deleteTask(taskId) {
  ...
  const confirm = window.confirm(`Confirma que deseas eliminar la tarea`);

  if (confirm) {

    fetchAPI(`${apiURL}/users/219220991/tasks/${taskId}`, 'DELETE')
      .then(() => {
        getTasksData();
...
function createTask(task) {
  ...
  fetchAPI(`${apiURL}/users/219220991/tasks`, 'POST', task)
    .then(task => {
      closeAddTaskModal();
      getTasksData();
...
function updateTask(task) {
  ...
  fetchAPI(`${apiURL}/users/219220991/tasks/${task.id}`, 'PUT', task)
    .then(task => {
      closeUpdateTaskModal();
      getTasksData();
